...
export default function Match({match}) {
	...
	return (
		...
		{showPopup && (
			<div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-gray-700 bg-opacity-50 z-50 text-black">
			<div className="bg-white p-4 rounded-md shadow-md">
				<p className="text-lg font-medium mb-2">Match Details</p>
				<p>{match.player1.user ? match.player1.user.username : ""} vs {match.player2.user ? match.player2.user.username : ""}</p>
