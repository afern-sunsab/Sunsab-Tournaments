...
export default function Page() {
  ...
  const handleSubmit = async (e) => {
    ...
    const thumbnailRef = ref(
      strg,
      `thumbnails/tournaments/${(await getHighestID("tournaments")) + 1}`
    );
    await uploadBytes(thumbnailRef, file);
    const thumbnailUrl = await getDownloadURL(thumbnailRef);
    ...
    const newTournament = {
      ...tournament,
      close_date: converted_close_date,
    ...
    if (addRandomPlayers) await debugAddPlayers(newTournament);
    ...
    <main className="bg-sunsab-white text-sunsab-blue p-6 ml-28 mr-28">
      {user ? (
        <form className="flex flex-col space-y-4" onSubmit={handleSubmit}>
          ...
          <label htmlFor="entrant_limit" className="text-sm font-medium">
            Entrant Limit:
          </label>
          <input
            type="number"
          ...
          <div>
            <label htmlFor="addRandomPlayers" className="text-sm font-medium">
              Add Random Players:
            </label>
            <input
              type="checkbox"
